  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <header className="min-w-0">
        <p className="text-sm font-semibold text-brand">Edit sumber ASK</p>
        <h1 className="mt-1 break-all text-3xl font-bold">{document.title}</h1>
        <p className="mt-2 text-sm text-muted-foreground">Status review: <strong>{reviewLabels[document.review_status]}</strong>{document.reviewer_display_name ? ` · ${document.reviewer_display_name}` : ""}</p>
      </header>
      ...
      <section className="ui-panel p-5">
        ...
        {document.review_history.length === 0 ? <p className="mt-3 text-sm text-muted-foreground">Belum ada keputusan review.</p> : (
          <ol className="mt-4 space-y-4">{document.review_history.map((entry, index) => (
            <li key={`${entry.created_at}-${index}`} className="border-l-2 border-primary/40 pl-4 text-sm">
              <p className="font-semibold">{reviewLabels[entry.new_status]}</p>
              <p className="mt-1 text-muted-foreground">{entry.actor_display_name ?? "Aktor tidak tersedia"} · {new Intl.DateTimeFormat("id-ID", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Jakarta" }).format(new Date(entry.created_at))}</p>
              <p className="mt-1 whitespace-pre-wrap text-foreground">{entry.reason}</p>
            </li>
          ))}</ol>
        )}
      </section>

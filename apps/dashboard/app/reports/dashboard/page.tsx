...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (!village.is_active || village.activation_status !== "approved") return <div data-guide="dashboard-inactive" className="ui-panel mx-auto max-w-2xl p-6"><h1 className="text-2xl font-bold">Desa belum aktif</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">Lengkapi pengaturan dan periksa status aktivasi untuk membuka ringkasan layanan desa.</p><Link href="/reports/settings" className="ui-primary mt-5">Buka pengaturan</Link></div>;
...
function InactiveVillage() {
  return <div data-guide="dashboard-inactive" className="ui-panel mx-auto max-w-2xl p-6"><h1 className="text-2xl font-bold">Desa belum aktif</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">Ringkasan tersedia setelah desa disetujui dan diaktifkan. Periksa status aktivasi pada pengaturan akun.</p><Link href="/reports/settings" className="ui-primary mt-5">Buka pengaturan</Link></div>;
}
